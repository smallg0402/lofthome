...
//首頁節點
const index_room_list = document.querySelector('.index-room-list');
console.log(index_room_list);
const modal = document.querySelector('.modal');
console.log(modal);
//rooms節點
let rooms_roomList = document.querySelector('.rooms_roomList')
...
bookAdultsGroup.forEach((item)=>{
  item.addEventListener('click',function(e){
    ...
    if(e.target.getAttribute('class').indexOf('input-group-text-prepend') != -1) {
      bookAdultsNum --;
      console.log("-");
      
  
    }else if(e.target.getAttribute('class').indexOf('input-group-text-append') != -1){
      bookAdultsNum ++;
      console.log("+");
...
function booking(){
  ...
  axios.post(`${url}room/3Elqe8kfMxdZv5xFLV4OUeN6jhmxIvQSTyj4eTgIowfIRvF4rerA2Nuegzc2Rgwu`,{
    headers:{
...
function getdata(){
  ...
  console.log(path);
  console.log(path.indexOf("/index.html"));
  axios.get(`${url}rooms`,{
    headers:{
      Authorization: `Bearer ${token}`,
      Accept: "application/json"
        ...
        <div class="row-48 mt-3 card_body">
            <div class="col-48-6 d-flex flex-column justify-content-between">
                <div class="container-46 px-0">
                  <ul class="row-46">
                      <li class="col-46-6">
                          <p class="add-before-line">人數：${item.descriptionShort.GuestMax}</p>
                          <p class="add-before-line mt-3">床型：${item.descriptionShort.Bed}</p>  
                      </li>
                      <li class="col-46-6">
                          <p class="add-before-line">坪數：${item.descriptionShort.Footage}平方公尺
                          </p>  
                          <p class="add-before-line mt-3">餐點：附早餐</p>  
            ...
            <div class="d-flex flex-column col-48-5 ml-auto justify-content-between pt-3">
                  ...
                  <div class="align-self-end">
                      <a href="" class="btn rooms-btn-more bg-light text-dark mr-2" data-roomno=${roomNo} data-roomnd=${item.id}>MORE +</a>
...
//進行第一次頁面渲染
  ...
  for(let i=0 ;i<roomAmenitiesArray.length;i++){
    ...
    roomAmenitiesArray[i].innerHTML = `<ul class="row-12" >`+amenitiesStr+`</ul>`;
